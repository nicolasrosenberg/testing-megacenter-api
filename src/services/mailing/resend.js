...
function generateMoveInEmailHTML(data, location) {
	return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Sign your lease agreement</title>
<style>
body{margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background-color:#f8f9fa}
.container{max-width:600px;margin:0 auto;background:#fff}
.header{text-align:center;padding:48px 40px 40px}
...
.content{padding:0 40px 48px}
h1{font-size:30px;font-weight:500;color:#1a1a1a;margin:0 0 12px 0;line-height:1.3;text-align:center;letter-spacing:-0.5px}
.subtitle{font-size:17px;color:#6b7280;margin:0 0 32px 0;line-height:1.6;text-align:center}
.cta-box{background:#eff6ff;border:2px solid #0042d4;border-radius:16px;padding:32px;margin-bottom:48px;text-align:center}
.cta-box h2{font-size:24px;font-weight:600;color:#1a1a1a;margin:0 0 8px 0}
.cta-box p{font-size:16px;color:#6b7280;margin:0 0 24px 0}
.cta-button{display:inline-block;background:#0042d4;color:#fff!important;text-decoration:none;padding:16px 48px;border-radius:8px;font-size:18px;font-weight:600;transition:background 0.2s}
.cta-button:hover{background:#0056f5}
.section-title{font-size:20px;font-weight:600;color:#1a1a1a;margin:32px 0 20px 0}
.info-table{width:100%;font-size:16px;color:#212529;background:#fff;border:1px solid #e5e7eb;border-radius:12px;margin-bottom:24px;overflow:hidden}
.info-table tr{border-bottom:1px solid #f3f4f6}
.info-table tr:last-child{border-bottom:none}
.info-table td{padding:12px 20px;vertical-align:top}
.label{color:#6b7280;text-align:left;font-weight:500;font-size:15px}
.value{color:#1a1a1a;text-align:right;font-weight:500;font-size:15px}
.highlight{font-weight:700;color:#0042d4;font-size:18px}
.info-table a{color:#0042d4;text-decoration:none;font-weight:500}
.price-row{background:#f9fafb}
.total-row{background:#eff6ff}
.total-row .label{color:#1a1a1a;font-weight:600;font-size:17px}
.total-row .value{color:#0042d4;font-weight:700;font-size:20px}
.date-range{font-size:13px;color:#9ca3af;display:block;margin-top:2px}
.footer{width:100%;font-size:16px;padding:48px 40px;color:#fff;background:linear-gradient(135deg,#0042d4 0%,#0056f5 100%)}
.footer table{width:100%}
.footer-logo{display:block;border:0;max-width:160px;height:auto}
.header-table{width:100%;margin-bottom:32px}
...
<div class="header"><img src="cid:logo-header" alt="Megacenter" class="logo"></div>
<div class="content">
<h1>Your move-in is confirmed</h1>
<p class="subtitle">${data.firstName} ${data.lastName}, your payment of $${data.totalPaid} has been processed successfully. To complete your move-in, please sign your lease agreement.</p>
<div class="cta-box">
<h2>Sign your lease agreement</h2>
<p>To complete your move-in process, please review and sign the required lease documents. This will open a secure electronic signature page.</p>
<a href="${data.eSignUrl}" class="cta-button">Sign documents</a>
</div>
<h2 class="section-title">Unit information</h2>
<table class="info-table">
<tr><td class="label">Tenant Name</td><td class="value">${data.firstName} ${data.lastName}</td></tr>
<tr><td class="label">Lease Number</td><td class="value">#${data.leaseNumber}</td></tr>
<tr><td class="label">Unit number</td><td class="value">${data.unitName}</td></tr>
<tr><td class="label">Gate Access Code</td><td class="value highlight">${data.gateCode}</td></tr>
<tr><td class="label">Unit type</td><td class="value">${data.unitType}</td></tr>
<tr><td class="label">Unit size</td><td class="value">${data.unitSize}</td></tr>
<tr><td class="label">Move in date</td><td class="value">${data.moveInDate}</td></tr>
</table>
...
<table class="info-table">
<tr><td class="label">First Monthly Rent Fee<span class="date-range">${data.rentPeriod}</span></td><td class="value">$${data.firstMonthRent}</td></tr>
<tr><td class="label">Administrative Fee</td><td class="value">$${data.adminFee}</td></tr>
<tr><td class="label">First Month Insurance<span class="date-range">${data.insurancePeriod}</span></td><td class="value">$${data.firstMonthInsurance}</td></tr>
<tr class="total-row"><td class="label">Total due today</td><td class="value">$${data.totalPaid}</td></tr>
</table>
</div>
<div class="footer">
<table class="header-table">
...
async function sendMoveInConfirmation(data, locationCode) {
	try {
		// Get location data
		const locationSlug = getLocationSlug(locationCode);
		const location = locationSlug ? locationsData[locationSlug] : null;
		...
		logInfo("MailingService", "Sending move-in confirmation email", {
			email: data.email,
			leaseNumber: data.leaseNumber,
			location: location.name,
		});

		const htmlContent = generateMoveInEmailHTML(data, location);
		...
		const result = await resend.emails.send({
			from: "onboarding@resend.dev",
			to: data.email,
			subject: "Your Megacenter move-in is confirmed",
			html: htmlContent,
			attachments: [
				{
					content: logoHeader,
		...
		logInfo("MailingService", "Move-in email sent successfully", {
			emailId: result.data?.id,
		});

		return result;
	} catch (error) {
		logError("MailingService", "Failed to send move-in email", {
			error: error.message,
			email: data.email,
		});
		throw error;
