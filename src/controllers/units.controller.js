...
async function getUnitsGrouped(req, res) {
	...
	logInfo(
		"UnitsController",
		`getUnitsGrouped called for ${locationSlug} (${locationCode})`,
	);

	// Fetch data in parallel
	const [unitTypesResponse, discountsResponse] = await Promise.all([
		unitsService.getUnitTypePriceList(
			config.CHANNEL_TYPES.WEB_RATE,
			locationCode,
		),
...
async function getUnitById(req, res) {
	...
	logInfo(
		"UnitsController",
		`getUnitById called for unit ${unitId} at ${locationSlug} (${locationCode})`,
	);

	// Use helper to validate and get unit with discounts
	const result = await unitsHelper.validateAndGetUnitWithDiscounts(
		unitId,
		locationCode,
		true,
	);
